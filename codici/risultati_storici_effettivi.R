...
varianzatargetmsquare<-0.035 #in percentuale
...
#preparazione rendimenti effettivi
...
rendimentieffettivi<-qtrtoyear(rendimentitrimestrali)

system.time(for(i in index(pesi)) {
#i<-'2008 Q2'
			
#calcolo rendimenti effettivi portfolio	
reffettivi[as.yearqtr(i,format="%Y Q%q"),]<-pesi[as.yearqtr(i,format="%Y Q%q"),]%*%t(rendimentitrimestrali[as.yearqtr(i,format="%Y Q%q"),])
...
rendimentixcov<-window(rendimentisettimanali, start = start(rendimentisettimanali), end=as.Date(as.yearqtr(i))) #estrae i dati dall'inizio fino al periodo corrente
covarianze<-52*cov(rendimentixcov)


#annualizzazione
reffettivi[as.yearqtr(i,format="%Y Q%q"),]<-qtrtoyear(reffettivi[as.yearqtr(i,format="%Y Q%q"),])
rendimentitrimestrali[as.yearqtr(i,format="%Y Q%q"),1]<-qtrtoyear(rendimentitrimestrali[as.yearqtr(i,format="%Y Q%q"),1])


stdeveffettiva[as.yearqtr(i,format="%Y Q%q"),]<-(pesi[as.yearqtr(i,format="%Y Q%q"),]%*%covarianze%*%t(pesi[as.yearqtr(i,format="%Y Q%q"),]))^0.5

sharpeeffettivo[as.yearqtr(i,format="%Y Q%q"),]<-(reffettivi[as.yearqtr(i,format="%Y Q%q"),]-rendimentitrimestrali[as.yearqtr(i,format="%Y Q%q"),1])/stdeveffettiva[as.yearqtr(i,format="%Y Q%q"),]

msquare[as.yearqtr(i,format="%Y Q%q"),]<-rendimentitrimestrali[as.yearqtr(i,format="%Y Q%q"),1]+sharpeeffettivo[as.yearqtr(i,format="%Y Q%q"),]*varianzatargetmsquare
...
#creazione lista risultatistorici
risultatistorici<-list()
risultatistorici$pesi<-pesi
risultatistorici$sharpe<-sharpe
risultatistorici$rporto<-rporto
risultatistorici$stdev<-stdev
risultatistorici$msquare<-msquare
